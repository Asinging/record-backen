 ...
 *     // -or-
 *     return res.userNotFound(optionalData);
 * ```
 *
 * Or with actions2:
 * ```
...
module.exports = function userNotFound(optionalData) {
  ...
  if (optionalData === undefined) {
    sails.log.info('Ran custom response: res.userNotFound()');
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.info('Custom response `res.userNotFound()` called with an Error:', optionalData);
