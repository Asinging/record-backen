 ...
 * Example usage:
 * ```
 *     return res.maniResponse();
 *     // -or-
 *     return res.maniResponse(optionalData);
 * ```
 *
 * Or with actions2:
 * ```
...
module.exports = function maniResponse(optionalData) {
  ...
  if (optionalData === undefined) {
    sails.log.info('Ran custom response: res.maniResponse()');
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.info('Custom response `res.maniResponse()` called with an Error:', optionalData);
  ...
  // Set status code and send response data.
    ...
    let optionalData = JSON.stringify(optionalData)
